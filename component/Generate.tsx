...
export default function Generate(props: GenerateProps) {
  ...
  const buttonClassNames = `text-center bg- z-10 shadow-xl drop-shadow-lg ${(!face || !bg) ? "" :
    "bg-blue-600 hover:bg-blue-700 cursor-pointer p-2 rounded-xl "} transition-all px-8 py-2.5 rounded-lg gap-2 text-white flex items-center font-normal`;
  ...
  return (
    <div className={containerClassNames}>
      <button
        ...
        color='red'
        disabled={loading} >
        <p>{loading ? loadingText : 'Хувиргах'}</p>
        <div>
